...
def main() -> None:
    parser = argparse.ArgumentParser(description="Regenerate the reviewer teacher-bank ladder submissions.")
    parser.add_argument("--package-root", default="../generalization_submission_params")
    ...
    for step in config["steps"]:
        ...
        cmd = [
            ...
            *[str(path) for path in inputs],
            ...
            "--test-root",
            str(mpdd_root / "extracted/Test-MPDD-Elder/Elder"),
            "--personality-npy",
            str(mpdd_root / "extracted/Train-MPDD-Elder/Elder/descriptions_embeddings_with_ids.npy"),
            "--out-dir",
            str(step_out),
            "--gait-banks",
            "base,segment",
        ...
        if step["alpha_grid"] != "default":
            cmd.extend(["--alpha-grid", step["alpha_grid"]])
        subprocess.run(cmd, check=True, env={"PYTHONPATH": pythonpath})

    print(f"Regenerated ladder submissions under {out_root}")
